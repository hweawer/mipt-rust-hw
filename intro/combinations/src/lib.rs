...
pub fn combinations(arr: &[i32], k: usize) -> Vec<Vec<i32>> {
    ...
    let mut data: Vec<i32> = Vec::with_capacity(k);
    inner(arr, k, 0, &mut data, &mut res);
    res
}

fn inner(arr: &[i32], k: usize, idx: usize, acc: &mut Vec<i32>, res: &mut Vec<Vec<i32>>) {
    if k == 0 {
        res.push(acc.clone())
    } else if idx >= arr.len() {
        return;
        ...
        acc.push(arr[idx]);
        inner(arr, k - 1, idx + 1, acc, res);
        acc.pop();
        inner(arr, k, idx + 1, acc, res);
    }
}
...
mod test {
    ...
    fn small_unsorted() {
        assert_eq!(
            combinations(&[1, 1337, 42, 228, 7], 3),
            vec![
                vec![1, 1337, 42],
                vec![1, 1337, 228],
                vec![1, 1337, 7],

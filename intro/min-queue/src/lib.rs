...
impl<T: Clone + Ord + Debug> MinQueue<T> {
    ...
    pub fn push(&mut self, val: T) {
        self.regular.push_back(val.clone());
        while self.min.back().is_some() && *self.min.back().unwrap() > val {
            self.min.pop_back();
        }
    ...
    pub fn pop(&mut self) -> Option<T> {
        let res = self.regular.pop_front();
        if let Some(top) = &res {
            if self.min().is_some() && *top == *self.min().unwrap() {
                self.min.pop_front();
            }
        };
        res
